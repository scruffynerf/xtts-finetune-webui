...
def create_zip(folder_path, zip_name):
    ...
    shutil.make_archive(str(zip_path).replace('.zip', ''), 'zip', folder_path)
    return str(zip_path)


def get_model_zip(out_path):
    ready_folder = Path(out_path) / "ready"
    if ready_folder.exists():
        return create_zip(str(ready_folder), "optimized_model")
    return None


def get_dataset_zip(out_path):
    dataset_folder = Path(out_path) / "dataset"
    if dataset_folder.exists():
        return create_zip(str(dataset_folder), "dataset")
    return None
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="""XTTS fine-tuning base\n\n"""
        """
        Example runs:
    ...
    parser.add_argument(
        "--max_audio_length",
        type=int,
        help="Max permitted audio size in seconds. Default: 16",
        default=11,
    )
    ...
    with gr.Blocks(title=os.environ.get("APP_NAME", "Gradio")) as demo:
        with gr.Tab("1 - Data processing"):
            ...
            def preprocess_dataset(audio_path, audio_folder_path, language, whisper_model, out_path, train_csv, eval_csv, progress=gr.Progress(track_tqdm=True)):
                ...
                out_path = Path(out_path) / "dataset"
        ...
        with gr.Tab("2 - Fine-tuning XTTS Encoder"):
            ...
            def train_model(custom_model, version, language, train_csv, eval_csv, num_epochs, batch_size, grad_acumm, output_path, max_audio_length):
                ...
                ft_xtts_checkpoint = Path(exp_path) / "best_model.pth"
                shutil.copy(str(ft_xtts_checkpoint), str(ready_dir / "unoptimize_model.pth"))
                ft_xtts_checkpoint = ready_dir / "unoptimize_model.pth"
    ...
    demo.launch(
        ...
        # inweb=True,
        server_name="0.0.0.0" # I prefer to use this to allow me to access from other machines on my network
    )
